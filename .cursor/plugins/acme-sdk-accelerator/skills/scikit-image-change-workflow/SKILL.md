...
14. Before asking about commit or PR creation, get an independent verification pass against the verifier checklist in `agents/scikit-image-verifier.md`. In the IDE, invoke the custom `scikit-image-verifier` Agent and read its verdict. In cloud (where the custom Agent is not auto-loaded), dispatch a review subagent directed to apply that checklist verbatim, or apply it inline if no subagent can be dispatched; use the same `PASS`/`FAIL`/`BLOCKED` verdict either way.
